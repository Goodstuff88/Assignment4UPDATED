...
# Creating space for leaflet and table output 
  ...
  column(width = 12,
      leafletOutput("World_Map", height = 700)
         ),
 #  column(width = 3,
...
# 1.2 Data set used for mapping leaflet map
ncov_reactive_spatial_data <- ncov_new_Spatial

# 2. Specifying value boc information
...
# 3. Specifying the details for world map output
 

output$World_Map <- renderLeaflet({
     
Map_Data <- ncov_reactive_spatial_data %>% 
  filter(Date == input$date)
...
leaflet_Map <- Map_Data %>% 
     leaflet(options = leafletOptions(minZoom = 2.5)) %>% 
       addProviderTiles(provider = "CartoDB", options = providerTileOptions( noWrap = TRUE) ) %>%  
     ...
     if(input$Top10Death){
       leaflet_Map %>%
         addAwesomeMarkers(lng = Top10Deaths$Long, lat = Top10Deaths$Lat, icon = Death_icon)
        #popup = ~paste0("<b>","Deaths:", "</b>",Deaths)

     }

if(input$Top10Recov){
  leaflet_Map %>%
    addAwesomeMarkers(Top10Recoveries, lng = Top10Recoveries$Long, lat = Top10Recoveries$Lat, icon = Recovery_icon) 
                      
                      #popup = ~paste0("<b>","Recoveries:", "</b>",Recovered)
...
leaflet() %>% 
  addTiles() %>% 
  addAwesomeMarkers(lng = Top10Recoveries$Long, lat = Top10Recoveries$Lat, icon = Recovery_icon)
