...
# 1.5 Creating colour palets for Cumalative cases
...
# 1.7 Simplifying the geometry part of the ncov_new_spatial data frame for easy plotting

ncov_new_Spatial <- ms_simplify(ncov_new_Spatial)
...
pal <-  colorNumeric(palette = "YlOrRd", domain = c(0,log(Country_Zero_Less_df$Cum_Confirmed_Country)), na.color = "lightgoldenrodyellow" )
...
ncov_new_Spatial %>%
  ...
  addProviderTiles(provider = "CartoDB.Positron", options = providerTileOptions( noWrap = TRUE) ) %>% 
       addPolygons(weight = 1,
                   fillOpacity = 0.5,
                   color = ~pal(log(Cum_Confirmed_Country)),
     ...
     panel(
       ...
       column(width = 4,
            prettyRadioButtons(inputId = "table", # Creating radio button input
                            ...
                            choices = c("None", "Cases", "Deaths", "Recoveries", "Infected"),
                            shape = "round", 
                            status = "danger",
                            fill = TRUE, 
                            animation = "pulse")
...
# Options that displays the top 10 locations with the highest death count
...
# Options that displays the top 10 locations with the highest recovery count
...
# 2.4 Plotly output - Cummalative country/region plot
...
# 2.4.2 Plotly output - Frequecy plot of counntry/region
...
# Frequency plot of the people infected (tested positive) of the country/region   
  ...
  if(input$DataSelection == "Infected_Country"){
    ...
    geom_point(aes(text = paste0('</br> Infected:', freq_Infected,'</br> Date: ', Date, '</br> Country: ', Country_Region)), size = 1) + 
    geom_line()  +
    labs(y = "Infected (per day) ") +
    theme_bw() +
      theme(legend.position = "none") 
...
World_Map_test <-  ggplot() + 
  geom_sf(data = ncov_new_Spatial, fill = "gray48")

World_Map_test

ncov_Spatial_Simplify <- ms_simplify(ncov_new_Spatial)

World_Map_test2 <-  ggplot() + 
  geom_sf(data = ncov_Spatial_Simplify, fill = "gray48")
World_Map_test2


World_map_test3 <- ggplot() +
  geom_sf(data = World)

World_map_test3

Data_Table <-  ncov_new_FINAL_data %>%
      filter(Date == "2020-04-15") 
...
Data_Table <- Data_Table[, c(7,1,2,3,4,5,6)]
