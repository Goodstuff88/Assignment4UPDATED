...
# User interface: The things that are essentially going to be i your app. Basically makes the webpage and space for the elements that are going to be present.

ui <- fluidPage(
  # input functions: sliders, check boxes, radio buttons, action buttons etc.
  # ALWAYS have 1) inputId and 2) label.
  # Use help file to retrieve the arguments needed for each input function
  
  
  # output functions: Plots, tables, text etc.
  # ALWAYS have 1) outputId
  
)


# inputs: Useally what the user selects
# outputs: What you whant to show like plots , tables texts. The things that wil repsond when yout user changes a input


# When using the server function there are three rules (Reactivityy 101):
# 1) Save objects to display to output$*
# 2) Build objects to display with render*({ R code block})
# 3) Use the input values with input$* (to access the values the user chooses)
...
# 1. Render functions: render*({R code chunk}). Makes objects to display
# 2. Reactive functions: Basically used to rename a reactive object. the syntax is as follows: data <- reactive({input$*}). Very important: the reactive object created (data) is not a value but actually a function and therefore everytime it needs to be used it needs to be called data().
# 3. isolate function: Basically makes a reactive object non-reactive. The syntax is isolate({ }).
# 4. observeEvent function: Triggers code to run on server(your R consol). Useally used to get info from the interface that the the user does not know of. The syntax is observeEvent(input$*, {print(input$)})
# 5. eventReactive function: Basically a mixture of reactive function and observe event. The function only uses the reactive value when a condition is met (ie a button is clicked etc)
# 6. reactiveValues function: Creates a list of reactive values to use in you app. Basically creating a set of test values for your app. The syntax is: reactiveValue(data = rnorm(100)). Useally used with observEvent. NB for cumalative plot.

# Part 3 tut

#Contents:
# 1. Static elements: HTML tags
# 2. Layout: Organising your web-page in rows and columns
# 3. Panels: How differnt layers are stacked up on each other like tabs. Look at the ready-made panels
# 4. DashboardPage() http://rstudio.github.io?shinydashboard - Tut
# 5. CSS - styling your dashboard (look at getbootstrap.com)
# 6. NB! Customise your apps: http://shiny.rstudio.com/articles/css.html
