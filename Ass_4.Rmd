...
The data wrangling consist of importing, tidying and transforming the data set.

## Imporing data

The first step of data wrangling is the importing of the data.

```{r Loading covid 19 data, include=FALSE}
...
# Covid 19 data up to the 15/04/2020 ((Most code written for this data set))

ncov_new <- read.csv("https://raw.githubusercontent.com/Goodstuff88/Assignment4UPDATED/master/time_series_19_covid_combined_10-05-2020.csv",
...
The second and thrid steps of data wrangling is tiding and transforming the data. The variables of the "ncovr_new" data set will be investigated and the nesseccary changes/transformations will be made to get the data in a workable format.   


```{r Investigating variables and treating missing values in data sets, include=FALSE}
...
# Remark: Seems that all the missing vales is from the same location in Canada.         

ncov_new %>% 
  filter(is.na(Recovered)) %>% 
  distinct(Province.State)

# Remark: The following Province state, does not have recovery data. 

# It seems that there is a lack of data for Canada. The data is not complete. For simplicity it would be best to not work with the NA values. Therefore, the data associated with Canda will be omited. All three variables need to be known for data to be useful for further functions to operate correctly.
...
```{r Additions and transformations to data set, include=FALSE}
...
# Creating visual animation and web aplication

To create the visual animation and web aplication, additional elements need to be added to the data set. To creat the desired visual animation was neccessary to attach a geometry variable (spatial object) to the exsiting "ncov_new" data set. This was done by making use of the natural earth data.    

```{r Loading of spatial data, include=FALSE}
...
```{r Making neccessary changes to accomplish merging of ncov_new and World data frames, include=FALSE}
...
# Investigating if join was sucessful (No NA's should be present)

sum(is.na(Country_Test_df$Pop_Estimate)) 

# Remark: There are NA's present, meaning there were some mis-match in th merging operation of the country names. These country will be investigated. It is probably due to spelling that caused the NA's.
...
```{r Merging operation of spatial data, include=FALSE}

# Note: The following code chunk deals with creating a data frame that contains the necessery data that will be used to create plots of a world map.
...
The data is now in the correct format, therefore the development of the animations can commence.    


```{r  Intoduction to animation, include=FALSE}
...
```{r Creating animation of the world map, echo=FALSE, warning=FALSE, message=FALSE }
...
Var_labels <- c("Cases", "Deaths", "Recoveries", "Infections")
...
World_Map <- ggplot() + 
  ...
  scale_fill_gradient2(low="green", mid = "yellow", high="red", trans = "log10", midpoint = 3) +
  labs(title = "World map", subtitle = "Date: {current_frame}", fill = "Count", x = "Latitude", y = "Longitude" ) + 
  theme(plot.title = element_text(size = 20, face = "bold" , hjust = 0.5), plot.subtitle = element_text(size = 16, face = "bold", colour = "red", hjust = 0.5), axis.title.x = element_text(size = 16), axis.title.y = element_text(size = 16),  legend.title = element_text(size = 16), legend.text = element_text(size = 16), panel.grid.major = element_line(color = "gray65", linetype = "dashed", size = 0.5), panel.background = element_rect(fill = "aliceblue"), strip.text = element_text(size = 16, face = "bold")) +
  facet_wrap(~Variable,
...
World_Gif <- animate(World_Map_anim, fps = 5, width = 950, height = 950 )

# 5. Displaying Gif created
...
```{r Creating an animation for Africa, echo=FALSE, warning=FALSE, message=FALSE}
...
Map_Afr <- ggplot() + 
  ...
  scale_fill_gradient2(low="green", mid = "yellow", high="red", trans = "log10", midpoint = 1.8) +
...
```{r Creating an animation for Europe, echo=FALSE, warning=FALSE, message=FALSE}
...
Map_Eur <- ggplot() + 
  ...
  scale_fill_gradient2(low="green", mid = "yellow", high="red", trans = "log10", midpoint = 2.7) +
...
```{r Creating an animation for Asia, echo=FALSE, warning=FALSE, message=FALSE}
...
```{r Creating an animation for North America, echo=FALSE, warning=FALSE, message=FALSE}
...
# 4. Plotting North Anerica map using ggplot() 
...
```{r Creating an animation for Oceania, echo=FALSE, warning=FALSE, message=FALSE}
...
Map_Oce <- ggplot() + 
  ...
  scale_fill_gradient2(low="green", mid = "yellow", high="red", trans = "log10", midpoint = 2.1) +
...
```{r Creating an animation for South America, echo=FALSE, warning=FALSE, message=FALSE }
...
Map_SA <- ggplot() + 
  ...
  scale_fill_gradient2(low="green", mid = "yellow", high="red", trans = "log10", midpoint = 2.3) +
...
Most of the variables and tables created are in the correct format to support the functioning of the web aplication developed. In the intoduction code block some additional variables were created support the function of some elements in the dashboard. 

```{r Introduction to shiny dashboard development, include=FALSE}

# Note: The following blocks of code will be associated with the development of the final Shiny dashboard. 

# Steps:

# 1. Saving static variables that are going to be used in the shiny application
...
# 1.4 Filtering to select non-zero values for each variable
...
# 1.5 Creating colour pallete for the world map
...
# 1.6 Frequency data set of ncov cases

# 1.6.1 Bind continents data with rest of data


Complete_Country_Region_df <- rbind(Continents_data, ncov_new_FINAL_data) 

# 1.6.2  Create "day counter" (ref) for data set

ref <- data.frame("Date" = unique(Complete_Country_Region_df$Date), 
                  "Day" = c(1:109))

# 1.6.2  Merge ref with ncov_new_FINAL_data

merge <- left_join(Complete_Country_Region_df, ref, by = "Date")

# 1.6.3 Create new columns for individual frequency counts
...
#  1.6.4 Selecting ony the freqeuncy columns
...
Note: If the app is not runned using the R-script file (app.R) this code block can be used to launch the web aplication. 

```{r Defining ui and server of shiny dashboard, include=FALSE}
